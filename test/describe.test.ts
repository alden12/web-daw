...
describe('describeCommand', () => {
  ...
  it('uses the instrument/effect label even without context', () => {
    expect(describeCommand({ type: 'createTrack', instrumentType: 'supersaw', id: 't-1' })).toBe('Added Supersaw track');
    expect(describeCommand({ type: 'addEffect', hostId: 'h-1', effectType: 'tremolo', id: 'fx-1' })).toBe('Added Tremolo');
  });

  it('resolves ids to names when given a context', () => {
    const ctx = { name: (id: string) => (id === 't-1' ? 'Demo Organ' : undefined) };
    expect(describeCommand({ type: 'createTrack', instrumentType: 'organ', id: 't-1' }, ctx)).toBe('Added Organ track Demo Organ');
    expect(describeCommand({ type: 'addEffect', hostId: 't-1', effectType: 'tremolo', id: 'fx-1' }, ctx)).toBe('Added Tremolo to Demo Organ');
    expect(
      describeCommand({ type: 'addNotes', trackId: 't-1', notes: [{ id: 'n', pitch: 60, start: 0, length: 1, velocity: 1 }] }, ctx),
