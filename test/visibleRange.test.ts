...
const ROLL = { width: 32, viewportSize: 300, leadPx: 38 };

describe("clampIntoView", () => {
  it("leaves something already on screen where it was asked to go", () => {
    expect(clampIntoView({ ...ROLL, wanted: 120, scrollOffset: 0 })).toBe(120);
    expect(clampIntoView({ ...ROLL, wanted: 500, scrollOffset: 400 })).toBe(500);
  });
  ...
  it("pins to the right edge rather than following a note off it", () => {
    ...
    const placed = clampIntoView({ ...ROLL, wanted: 9000, scrollOffset: 0 });
    ...
    expect(placed + ROLL.width).toBe(ROLL.viewportSize - ROLL.leadPx);
  });
  ...
  it("allows for the sticky label column, which covers the near edge at every offset", () => {
    ...
    const withColumn = clampIntoView({ ...ROLL, wanted: 9000, scrollOffset: 0 });
    const without = clampIntoView({ ...ROLL, leadPx: 0, wanted: 9000, scrollOffset: 0 });
    expect(without - withColumn).toBe(ROLL.leadPx);
  });

  it("pins to the near edge when the note has been scrolled off to the left", () => {
    expect(clampIntoView({ ...ROLL, wanted: 40, scrollOffset: 600 })).toBe(600);
  });
  ...
  it("stays reachable in a viewport too narrow to hold it", () => {
    ...
    expect(clampIntoView({ wanted: 500, width: 32, viewportSize: 50, leadPx: 38, scrollOffset: 100 })).toBe(100);
  });
});
