...
import { voiceCapFor } from "./vocabulary";
...
export class GraphInstrument extends BaseInstrument {
  ...
  constructor(ctx: BaseAudioContext, store: ParamStore, def: GraphInstrumentDef) {
    ...
    // Every note runs its own copy of each custom-DSP block, so a voice with some plays fewer at once.
    this.maxVoices = voiceCapFor(def.voice);
