 ...
 *
 * ## The variable in front of `env()`
 *
 * Each value reads a custom property before falling back to the real inset. Nothing writes
 * those properties in a production build, so the fallback is what ships. They are there so the
 * padding can be looked at on a device that reports no insets, which is every phone *browser* -
 * see `safeAreaSimulation.ts`, the dev-only half that writes them.
 */
...
/** The custom property that overrides one edge. Named here because both halves need the name. */
export const simulationVariable = (edge: SafeAreaEdge) => `--sim-safe-${edge}`;

const inset = (edge: SafeAreaEdge) => `var(${simulationVariable(edge)}, env(safe-area-inset-${edge}))`;
